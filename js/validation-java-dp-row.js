$(document).ready(function(){
    $('#send_message').click(function(){
        ...
        if(error == false){
           ...
           $('#form_e11').prop("action", "https://docs.google.com/forms/u/0/d/e/1FAIpQLSc0BNWN35OLlmg7OpaaOLJHDJU9WkWEbRnd5rCqc16TO13ngw/formResponse");
...
function maxLengthCheck(object){
 ...
 function form_check(){
    ...
    if(regex1.test(name) && name.length > 1 )
    {
            ...
            if(regex.test(age) && age.length == 2 && age >=20 && age <36)
            {
                    ...
                    if(phone.substr(0, 3) == "010" && phone.length ==11 && regex.test(phone))
                    {
                            ...
                            if (location != null)
                                ...
                                else
                                {
                                    $('#send_message').css({transition:"1s"});
                                    $('#send_message').prop("disabled", true);
                                    $('#send_message').prop("value", "희망 지점 선택을 확인하세요.");
                                    $('#send_message').css({background:"#121212"});
                                    $('#send_message').css({cursor:"default"});        
                                }
            ...
            $('#send_message').css({transition:"1s"});
            $('#send_message').prop("disabled", true);
            $('#send_message').prop("value", "20세 ~ 35까지 가능합니다.");
            $('#send_message').css({background:"#121212"});
            $('#send_message').css({cursor:"default"});     
            }
...
$(function(){
 $('#name,#phone,#location,#age,#message,#agree11,#now-wg,#hope-wg,#height').bind("keyup click change",form_check);
 $('#name,#phone,#location,#age,#message,#agree11,#now-wg,#hope-wg,#height').bind("keyup click change",form_check1);
